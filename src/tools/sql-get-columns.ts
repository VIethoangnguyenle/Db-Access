...
export function registerSqlGetColumns(server: McpServer, source: Source): void {
    server.tool(
        "sql_get_columns",
        "Get detailed schema of columns for a specific table in a relational database (Oracle or PostgreSQL). Includes column names, types, nullability, and comments.",
        {
        ...
        async ({db_name, table_name}) => {
            try {
                ...
                const result = await driver.getTableColumns(db_name, table_name);
                return {
